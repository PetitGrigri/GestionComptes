...
class CompteOrCategorieVoter extends Voter
{
    ...
    {
    	if ((($subject instanceof \FGS\GestionComptesBundle\Entity\Compte) || ($subject instanceof \FGS\GestionComptesBundle\Entity\CategorieMouvementFinancier)) 
    ...
    protected function voteOnAttribute($attribute, $subject, TokenInterface $token)
    {
		switch ($attribute) {
			case (self::PROPRIETAIRE) : //on regarde si l'utilisateur est le propriétaire du compte ciblé par le mouvement financier
										if (($token->getUser() !== null) && ($token->getUser()->getId() === $subject->getUtilisateur()->getId())) {
											return true;
										}
										break;						
		}
